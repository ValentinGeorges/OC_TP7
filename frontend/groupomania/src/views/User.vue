...
export default {
    ...
    mounted () {
        ...
        axios.get('http://localhost:8082/api/members/' + urlId + '/posts', 
        {
            headers: {
        ...
        axios.get('http://localhost:8082/api/members/' + urlId, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
    ...
    methods: {
        deleteAccount() {
            ...
            this.$confirm("Cette action est irréversible, voulez-vous vraiment supprimer votre compte ?").then(() => {
                axios.delete('http://localhost:8082/api/members/' + urlId, 
                {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
